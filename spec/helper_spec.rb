...
describe Virtualman::Interactive::Helper do
	...
	describe "#choose" do
		it "should return the right VM" do
			Virtualman::Interactive::Menu.should_receive(:unic_run).with(["Debian","2nd vm","exit"]).and_return("Debian","2nd vm","exit")
			@repl.choose("source_vms").should eql({"name" => "Debian", "snapshot" => "test with space"})
			@repl.choose("source_vms").should eql({"name" => "2nd vm", "snapshot" => "test2"})
	...
	describe "#choose_vm" do
		before do
			Virtualman::Interactive::Menu.should_receive(:unic_run).with(["Debian","2nd vm","exit"]).and_return("Debian","2nd vm")
		end
		...
		it "should return the right VM" do
			...
			vm_choosed = @repl.choose_vm "source_vms"
			vm_choosed.should be_an_instance_of Virtualman::Vm
			vm_choosed.name.should eql "2nd vm"
		end
	end
	...
	describe "#clone_vm" do
		...
		it "shoudl properly clone a VM with a spaced name" do
			Virtualman::Interactive::Menu.should_receive(:unic_run).with(["Debian","2nd vm","exit"]).and_return("2nd vm")
			Virtualman::Interactive::Menu.should_receive(:ask).and_return("Clone2")
			Virtualman::Vm.any_instance.should_receive(:manage).with("clonevm", "--snapshot \"test2\" --name \"Clone2\" --register").and_return(1)
			cloned_vm = @repl.clone_vm
			cloned_vm.should be_an_instance_of Virtualman::Vm
			cloned_vm.name.should eql "Clone2"
		end
	end
